...
Tree.nodeNames = 1:(2*sl-1); % Does not appear to be used here
Tree.kids = zeros(2*sl-1,2);
Tree.numkids = ones(2*sl-1,1);
Tree.node_y1c1 = zeros(hidden_size,2*sl-1);
Tree.node_y2c2 = zeros(hidden_size,2*sl-1);
Tree.freq = zeros(2*sl-1,1); % Does not appear to be used here
Tree.nodeFeatures = [words_embedded zeros(hidden_size,sl-1)];
Tree.nodeFeatures_unnormalized = [words_embedded zeros(hidden_size,sl-1)];
Tree.nodeDelta_out1 = zeros(hidden_size,2*sl-1);
Tree.nodeDelta_out2 = zeros(hidden_size,2*sl-1);
Tree.parentDelta = zeros(hidden_size,2*sl-1);
...
if updateWcat
    ...
    Tree.catDelta_out = zeros(hidden_size,2*sl-1);
